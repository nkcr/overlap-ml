...
def train(epoch, hidden_init):
    ...
    ntokens = ds.ntokens
    if args.use_repetitions == 1:
        logger.info('init hidden at each epoch')
    ...
    for data, targets in train_seq():
        ...
        hidden = repackage_hidden(hidden_init)
        optimizer.zero_grad()
...
try:
    ...
    for epoch in range(1, args.epochs):
        ...
        print('repetitions {}'.format(args.use_repetitions))
        print(epoch % args.use_repetitions == 0)

        train(epoch, hidden_glob)

        if args.use_repetitions > 1 and epoch % args.use_repetitions == 0:
            logger.info('hidden state initialized at epoch: {}'.format(epoch))
            hidden_glob = model.init_hidden(ds.batch_size)
        ...
        if epoch % args.use_repetitions == 0:
