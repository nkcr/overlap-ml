...
from awd.embed_regularize import embedded_dropout
from awd.locked_dropout import LockedDropout
from awd.weight_drop import WeightDrop

from awd.embedding_mul import EmbeddingMul
...
class RNNModel(nn.Module):
    ...
    def forward(self, input, hidden, return_h=False):
        masked_embed_weight, padding_idx = embedded_dropout(self.encoder,
                    dropout=self.dropoute if self.training else 0)  # nopep8
        emb = self.embed_func(input, masked_embed_weight,
                              padding_idx, self.encoder.max_norm,
                              self.encoder.norm_type,
                              self.encoder.scale_grad_by_freq,
                              self.encoder.sparse)
        #emb = self.idrop(emb)
