...
from mos.embed_regularize import embedded_dropout
from mos.locked_dropout import LockedDropout
from mos.weight_drop import WeightDrop
...
class RNNModel(nn.Module):
    ...
    def __init__(self, rnn_type, ntoken, ninp, nhid, nhidlast, nlayers,
        ...
        self.rnns = [torch.nn.LSTM(ninp if l == 0 else nhid, nhid if l !=
                                   nlayers - 1 else nhidlast, 1, dropout=0) for l in range(nlayers)]
        if wdrop:
            self.rnns = [WeightDrop(
                rnn, ['weight_hh_l0'], dropout=wdrop if self.use_dropout else 0) for rnn in self.rnns]
        self.rnns = torch.nn.ModuleList(self.rnns)

        self.prior = nn.Linear(nhidlast, n_experts, bias=False)
        self.latent = nn.Sequential(
            nn.Linear(nhidlast, n_experts*ninp), nn.Tanh())
    ...
    def forward(self, input, hidden, return_h=False, return_prob=False):
        ...
        latent = self.lockdrop(
            latent, self.dropoutl if self.use_dropout else 0)
        ...
        prob = nn.functional.softmax(
            logit.view(-1, self.ntoken), -1).view(-1, self.n_experts, self.ntoken)
