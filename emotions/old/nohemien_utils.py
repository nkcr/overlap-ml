...
class AudioWindowDataset(Dataset):
    ...
    def __init__(self, samples, labels, ids, window_size=500, step_size=1, how='random-no_step', batch_size=8):
        assert how in ['complete_random','local_order','standard_order','total_order']
        self.src = samples
        self.trg = labels
        self.ids = ids
        self.how = how
    ...
    def create_order(self):
        ...
        for i, x in enumerate(self.src):
            ...
            if len(x) > self.window_size + step_size:
                n_steps = int(self.window_size/step_size) + 1 if total_order else 1
        ...
        if complete_random:
            ...
            indexes = [[i + j*column_length for j in range(self.batch_size)] for i in range(column_length)]
            indexes = [x for sublist in indexes for x in sublist]
            return_list = [return_list[i] for i in indexes]

            if local_order:
                list_of_list = [return_list[i:i+self.batch_size] for i in range(0, len(return_list), self.batch_size)]
...
def load_nohemien(data_path, cv):
    # Loading from cross validation splits
    print('[INFO] Loading data...')
    train_sessions_f = [np.load(data_path + '/sess{}_features.npy'.format(i)) for i in range(1,6) if i != cv]
    train_sessions_l = [np.load(data_path + '/sess{}_label.npy'.format(i)) for i in range(1,6) if i != cv]
    ...
    # Labels are stored for different purposes, here we are interested only in classification so we take the first element
    ...
    neutral_feat = [sublist for i, x in enumerate(train_labels) if x == 'neu' for sublist in train_features[i]]
    mean_feat = np.mean(neutral_feat, axis=0)
    std_feat = np.std(neutral_feat, axis=0)
    train_features = [[(sublist - mean_feat) / (std_feat + 0.0001) for sublist in x] for x in train_features]
    test_features = [[(sublist - mean_feat) / (std_feat + 0.0001) for sublist in x] for x in test_features]
    ...
    test_ids = np.arange(len(train_features), len(test_features) + len(train_features))
...
def to_categorical(label_list):
    ...
    labels_set = sorted(list(set([x for sublist in label_list for x in sublist])))
